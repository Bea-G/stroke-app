...
stroke_by_glucose = df.groupby('glucose_group')['stroke'].mean().reset_index()
...
stroke_by_smoking = df.groupby('smoking_status')['stroke'].mean().reset_index()
...
with col1:
    stroke_by_hyper = df.groupby('hypertension')['stroke'].mean().reset_index()
    ...
    st.pyplot(fig4)
with col2:
    stroke_by_heart = df.groupby('heart_disease')['stroke'].mean().reset_index()
...
if len(user_filter) >= 10:
    ...
    if len(user_filter) > 0:
        st.warning("Limited data: Only {} similar individuals found.".format(len(user_filter)))
        st.info(f"Instead, based on your age group ({fallback_group}), the average stroke rate is: **{group_risk[0]:.2%}**")
    else:
        st.info(f"No matches found. Based on your age group ({fallback_group}), the average stroke rate is: **{group_risk[0]:.2%}**")
